...
OUTPUT
--------------------------------------------------------------------------------
  CONTRACT-COMPLIANT (Race Results Data Contract v2.7.0):
  <outdir>/<series_id>__<season>__schedule__<scraped_date>.csv           (§7)
                                 One row per SESSION of a round that has ALREADY
                                 RUN. session_datetime_local required. 13 columns.
  <outdir>/<series_id>__<season>__upcoming_schedule__<scraped_date>.csv  (§7a)
                                 One row per announced SESSION of a round that has
                                 NOT yet run. session_date required;
                                 session_datetime_local optional (blank until the
                                 running time is published). 13 columns.

  Both match the contract exactly: column names + order, filename (§1.1),
  UTF-8/no-BOM/Unix-LF (§1.2), series_id 68, circuit_id from dim_circuits,
  venue-offset local times (§3.4), the 18:00 rule (SESSION_TIME_001).

  Which round goes where: a round is routed by whether its race weekend has
  finished relative to `today` (the is_upcoming flag) -- completed -> schedule,
  future -> upcoming_schedule. A round with no published session timetable
  produces no rows in either file.
  ...
  un-raced round) nor a schedule/upcoming_schedule row (no driver columns). It is
  kept as a separate, clearly-labelled reference output; fields that DO have a
  contract counterpart still follow the contract's conventions (see
  BORROWED_FIELDS).
...
# =============================================================================
# 1b. CONTRACT CONFIGURATION (Race Results Data Contract v2.7.0, §7 / §7a)
# =============================================================================
...
#   (scraped_at was borrowed previously but REMOVED here: contract 2.6.0 deleted
#    it from every file, so there is no longer a counterpart to borrow.)
#
...
def _warn_if_late_session(local_ts: str, label: str, warnings: list) -> None:
    """
    SESSION_TIME_001: no session may start at or after 18:00 local WALL-CLOCK
    time (the value as written, before the offset; boundary inclusive). We never
    alter a real time to dodge this -- we emit it and warn that the file will be
    rejected, exactly as the contract intends (§3.4).
    """
    match = re.search(r"T(\d{2}):(\d{2}):", local_ts or "")
    if match:
        hour, minute = int(match.group(1)), int(match.group(2))
        if (hour, minute) >= (18, 0):
            warnings.append(
                f"{label}: starts {hour:02d}:{minute:02d} local (>= 18:00) -- this "
                f"row FAILS SESSION_TIME_001; the file cannot be submitted with it. "
                f"If the time looks wrong, suspect a double-applied offset (§3.4)."
            )


def _classified_sessions(round_dict: dict, warnings: list):
    """
    Yield (session_type, session_number, session_dict) for each session of a
    round that maps to a contract session_type.

    Shared by both the `schedule` and `upcoming_schedule` builders so numbering
    and exclusion behave identically:
      * session_type via classify_session (`practice`|`qualifying`|`race`).
      * "Official Test" (any "test") excluded silently; other unrecognized
        labels skipped WITH a warning.
      * session_number = explicit trailing integer in the label ("Race 2" -> 2),
        else a running count within the (round, session_type), in listed order.
    """
    type_counter = {"practice": 0, "qualifying": 0, "race": 0}
    for session in round_dict.get("sessions") or []:
        raw_name = session.get("name") or ""
        session_type = classify_session(raw_name)
        if session_type is None:
            if "test" not in raw_name.lower():
                warnings.append(
                    f"Round {round_dict.get('round_number')}: unrecognized session "
        ...
        yield session_type, session_number, session


def build_schedule_rows(dataset: dict) -> tuple:
    """
    Build `schedule` rows (§7) for rounds whose event has ALREADY RUN
    (is_upcoming == False). On a completed event the running time is a fact of
    record, so `session_datetime_local` is required; a session without a usable
    time is skipped with a warning.

    Returns (rows, warnings). §7 column order, 13 columns (scraped_at was
    removed in contract 2.6.0).
    """
    ...
    for r in dataset["rounds"]:
        if r.get("is_upcoming"):
            continue  # future events belong in upcoming_schedule (§7a)
        ...
        if circuit is None:
            warnings.append(
                ...
                f"in dim_circuits (CIRCUIT_BY_NAME) -- rows skipped; escalate (§10)."
            )
            continue

        for session_type, session_number, s in _classified_sessions(r, warnings):
            local_ts = local_timestamp_with_offset(
                s.get("date"), s.get("time"), circuit["tz"]
            )
            if local_ts is None:
                warnings.append(
                    f"Round {r.get('round_number')}: session {s.get('name')!r} has no "
                    f"usable time -- skipped (schedule requires session_datetime_local)."
                )
                continue
            _warn_if_late_session(
                local_ts, f"Round {r.get('round_number')} {s.get('name')!r}", warnings)
...
def build_upcoming_schedule_rows(dataset: dict) -> tuple:
    """
    Build `upcoming_schedule` rows (§7a) for rounds that have NOT yet run
    (is_upcoming == True) -- what the series has announced.

    `session_date` (the announced day) is REQUIRED; `session_datetime_local`
    (the running time) is OPTIONAL and left EMPTY when the site has not published
    a time yet -- never guessed (§7a). When both are present their wall-clock
    day agrees by construction (SESSION_DATE_001), since both derive from the
    same parsed session date.

    Returns (rows, warnings). §7a column order, 13 columns.
    """
    season_label = dataset.get("season_label")
    collector = dataset.get("collector") or DEFAULT_COLLECTOR
    series_id = dataset.get("series_id", CONTRACT_SERIES_ID)
    ...
    rows, warnings = [], []
    for r in dataset["rounds"]:
        if not r.get("is_upcoming"):
            continue  # completed events belong in schedule (§7)
        if not (r.get("sessions")):
            continue  # round announced but no session-level timetable yet
        circuit = resolve_circuit(r.get("circuit"))
        if circuit is None:
            warnings.append(
                f"Round {r.get('round_number')} ({r.get('circuit')!r}): circuit not "
        ...
        for session_type, session_number, s in _classified_sessions(r, warnings):
            ...
            if not session_date:
                warnings.append(
                    f"Round {r.get('round_number')}: session {s.get('name')!r} has no "
                    f"announced day -- skipped (upcoming_schedule requires session_date)."
                )
                continue

            # Time is optional: emit it only if the site actually published one.
            local_ts = ""
            if s.get("time"):
                built = local_timestamp_with_offset(session_date, s.get("time"), circuit["tz"])
                if built:
                    local_ts = built
                    _warn_if_late_session(
                        local_ts, f"Round {r.get('round_number')} {s.get('name')!r}", warnings)
...
def _write_contract_csv(path: str, columns: list, rows: list) -> None:
    """Write a contract CSV: UTF-8, no BOM, Unix LF, single header, §2.6 order."""
...
def _print_notes(tag: str, warnings: list) -> None:
    if warnings:
        print(f"\n[{tag}] contract notes (nothing written silently dropped):",
              file=sys.stderr)
...
def write_schedule_csv(dataset: dict, outdir: str) -> str:
    """
    Write the `schedule` file (§7) -- completed rounds. 13 columns, exact order.
    Filename: <series_id>__<season_label>__schedule__<scraped_date>.csv (§1.1).
    """
    columns = [  # §7 canonical order -- do not reorder (COLUMN_NAMES_002)
    ...
    series_id = dataset.get("series_id", CONTRACT_SERIES_ID)
    path = os.path.join(outdir, f"{series_id}__{season_label}__schedule__{scraped_date}.csv")
    _write_contract_csv(path, columns, rows)
    _print_notes("schedule", warnings)
    return path


def write_upcoming_schedule_csv(dataset: dict, outdir: str) -> str:
    """
    Write the `upcoming_schedule` file (§7a) -- announced, not-yet-run rounds.
    13 columns, exact order (session_date required; session_datetime_local
    optional; no planned_duration_minutes).
    Filename: <series_id>__<season_label>__upcoming_schedule__<scraped_date>.csv.
    """
    columns = [  # §7a canonical order
        "series_id", "season_label", "round_number", "session_type",
        "session_number", "circuit_id", "session_date", "session_datetime_local",
        "sport", "discipline", "category", "source_url", "source_collector",
    ]
    rows, warnings = build_upcoming_schedule_rows(dataset)
    scraped_date = (dataset.get("scraped_at") or "")[:10] or dataset.get("as_of_date")
    season_label = dataset.get("season_label") or "unknown"
    series_id = dataset.get("series_id", CONTRACT_SERIES_ID)
    path = os.path.join(
        outdir, f"{series_id}__{season_label}__upcoming_schedule__{scraped_date}.csv")
    _write_contract_csv(path, columns, rows)
    _print_notes("upcoming_schedule", warnings)
    return path
...
def write_roster_csv(dataset: dict, outdir: str) -> str:
    ...
    the contract's own convention (see BORROWED_FIELDS at the foot of this file):
      ...
      * source_url / source_collector            (lineage, §5.6)
      * text cleaned per §3.5; booleans per §3.3.

    (scraped_at was dropped: contract 2.6.0 removed it from every file, so it no
    longer has a contract counterpart to borrow -- the scrape date now lives only
    in the contract filenames.)
    """
...
def main(argv=None) -> int:
    ...
    if not args.no_files:
        ...
        written = [
            ...
            write_schedule_csv(dataset, args.outdir),           # §7  completed rounds
            write_upcoming_schedule_csv(dataset, args.outdir),  # §7a announced rounds
